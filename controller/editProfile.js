...
module.exports.EditUserProfile = async (req, res) => {
    try {
        ...
        if (!u_currency || (!userData[0]?.u_gender && !u_gender) || !u_address || !u_mobile || !u_country) {
            return res.send({ result: false, message: "Please fill all the fields" })
        }
        let gender = userData[0]?.u_gender ? userData[0]?.u_gender : u_gender
        let update = await model.EditProfile(user_id, u_currency.toUpperCase(), gender, u_address, u_mobile, u_country)
